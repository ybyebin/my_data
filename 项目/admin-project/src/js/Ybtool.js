 ...
 * @Last Modified by: yb
 * @Last Modified time: 2018-03-05 10:23:42
 */

/******************查询***********************
 * 
 *          
 * setStates------------>全局状态管理-------设置值
 * getStates------------>全局状态管理-------根据 key 取值
 * getStatusData-------->全局状态管理-------获取全部状态数据
 * timeFormat----------->时间格式化函数
...
! function(win) {
    ...
    //全局配置
    Ybtool.prototype.config = function(options) {
        options = options || {};
    ...
    //记录基础数据
    // Ybtool.prototype.cache = config;

    /*
     *  全局状态管理-------设置值
     * @param options {object} 键值对
     * @param target {object} 属性值为对象的属性，只在函数实现时递归中传入
     * @desc 通过传入键值对的方式修改state树
     */
    Ybtool.prototype.setStates = function(options, target) {
     ...
     */
    Ybtool.prototype.getStates = function(key) {
        var states = this.data;
        return states[key] ? states[key] : undefined;
    };
    // 全局状态管理-------获取 全部 状态数据
    Ybtool.prototype.getStatusData = function() {
        return this.data;
    };
     ...
     */
    Ybtool.prototype.individuationTime = function(time) {
    ...
    // 倒计时
    Ybtool.prototype.countDown = function(num, fun) {
        for (var i = 0; i <= num; i++) {
            (function(i) {
                setTimeout(function() {
                    console.log(i)
     ...
     * @return {} number 
     */
    Ybtool.prototype.randomNumber = function(n, m, type) {
    ...
    Ybtool.prototype.randomString = function(length) {
     ...
     * @returns 
     */
    Ybtool.prototype.getJsonKey = function(json) {
        return Object.keys(json);
    };

    //阻止事件冒泡
    Ybtool.prototype.stope = function(thisEvent) {
     ...
     * @returns 
     */
    Ybtool.prototype.deepClone = function(obj) {
     ...
     * @returns 
     */
    Ybtool.prototype.debounce = function(fn, delay, options) {
        if (!options) {
            options = {};
        }
        var delay = delay || 300;
    ...
    //css外部加载器
    Ybtool.prototype.link = function(href, cssname) {
        var that = this,
     ...
     */
    Ybtool.prototype.goTop = function(duration) {
     ...
     * @param {any} handler 
     */
    Ybtool.prototype.addHandler = function(element, type, handler) {
     ...
     * @param {any} handler 
     */
    Ybtool.prototype.removeHandler = function(element, type, handler) {
    ...
    /**
     * 
     * js  原生对象  判断是否包含某 class
     * 
     * @param {object} obj 
     * @param {string} className 
     * @returns 
     */
    Ybtool.prototype.jsObjHasClass = function(obj, className) {
        return jsObjHasClass(obj, className);
    }

    // 判断输入字符串是否合法
    Ybtool.prototype.strRegeMatch = function(str) {
    ...
    // 获取地址栏参数
    Ybtool.prototype.getRequest = function() {
    ...
    // 获取 浏览器版本和序号-1
    Ybtool.prototype.getExplore = function() {
        var Sys = {};
    ...
    // 获取浏览器 名称
    Ybtool.prototype.getExploreName = function() {
        return getExploreName();
    };
    // 检测浏览器语言
    Ybtool.prototype.getBrowserLanguage = function() {
    ...
    Ybtool.prototype.judgeMachine = function() {
        var type;
    ...
    //设备信息
    Ybtool.prototype.device = function(key) {
    ...
    //自适应rem初始化(淘宝~)
    Ybtool.prototype.setHtmlRem = function() {
        var b = document;
        var a = {};
    ...
    //隐式转换
    Ybtool.prototype.yszzAdd = function() {
    ...
    // 简单分页
    Ybtool.prototype.pageInit = function(config) {
        var option = {
            id: '',
            pageCount: 0,
            current: 0,
    ...
    //获取节点的style属性值
    Ybtool.prototype.getStyle = function(node, name) {
     ...
     * });
     */
    Ybtool.prototype.imgLoader = function(config) {
        this.option = {
...
//     resources: [
//         // 新增
//         'https://ss0.bdstatic.com/k4oZeXSm1A5BphGlnYG/newmusic/suibiantingting.png',
//         './image/clothes.png'
//     ],
//     onStart: function(total) {
//         console.log('start:' + total);
